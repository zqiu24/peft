    ...
    def backward(ctx, grad_output):
        ...
        if ctx.needs_input_grad[0]:
            ...
            inverse_permutation[permutation] = torch.arange(
                len(permutation), device=permutation.device, dtype=permutation.dtype
            )
        ...
        # Return gradients for each input of forward: (x, permutation)
        return grad_input, None

def triton_permute(x, permutation):
    return TritonPermute.apply(x, permutation)
...
def benchmark(shape, device='cuda', repeats=100):
    ...
    for _ in range(10):
        ...
        y = triton_permute(x, permutation)
        y.backward(grad_output, retain_graph=True)
...
def run_benchmarks():
    ...
    for shape in shapes:
        ...
        pt_time, tr_time = benchmark(shape)
        results['pytorch'].append(pt_time)
        results['triton'].append(tr_time)
        print(f"PyTorch: {pt_time:.3f} ms | Triton: {tr_time:.3f} ms | Speedup: {pt_time/tr_time:.2f}x")
    
    # Plot results
    x_labels = [f"{s[0]}x{s[1]}x{s[2]}" for s in shapes]
    
    plt.figure(figsize=(10, 5))
    plt.plot(x_labels, results['pytorch'], label='PyTorch', marker='o')
    plt.plot(x_labels, results['triton'], label='Triton', marker='o')
    plt.title('Permutation Operation Performance (Forward + Backward)')
    plt.ylabel('Time (ms)')
    plt.legend()
    plt.tight_layout()
    plt.savefig('permutation_benchmark.png')
    plt.show()
...
if __name__ == '__main__':
    ...
    # Forward check
    y_triton = triton_permute(x, permutation)
    y_pytorch = pytorch_permute(x, permutation)
