...
@jit
def _qoft_sdd_kernel(A, B, C,  #
                stride_ha, stride_ma, stride_ak,  #
                stride_hb, stride_bk, stride_nb,  #
                stride_hc, stride_mc, stride_nc,  #
                K, grid_offset, lut,  #
...
def qoft_sdd_matmul(a, b, trans_a, trans_b, trans_c, spdims, block, lut, widths, out=None):
    if a.stride(-2) != 1 and a.stride(-1) != 1:
        a = a.contiguous()
    ...
    if out is None:
        ...
        c = torch.empty((lut.shape[0], block, block), dtype=a.dtype, device=a.device)
    else:
        # assert out.shape == (a.shape[0], lut.shape[0], block, block)
        assert out.shape == (lut.shape[0], block, block)
        c = out
    # grid = [c.shape[1], 1, c.shape[0]]
    grid = [c.shape[0], 1]
    _qoft_sdd_kernel[grid](
        a, b, c,  #
        # a.stride(0), a.stride(1), a.stride(3 if trans_a else 2), a.stride(2 if trans_a else 3),  #
        a.stride(0), a.stride(2 if trans_a else 1), a.stride(1 if trans_a else 2),
        # b.stride(0), b.stride(1), b.stride(3 if trans_b else 2), b.stride(2 if trans_b else 3),  #
        b.stride(0), b.stride(2 if trans_b else 1), b.stride(1 if trans_b else 2),
...
def qoft_sdd_lut(layout, block, device):
...
@jit
# @triton.jit
def _qoft_dsd_kernel(A, B, C,  #
                stride_ha, stride_am, stride_ak,  #
                stride_hb, stride_bk, stride_bn,  #
                stride_hc, stride_cm, stride_cn,  #
                DS0, DS1, lut,  #
                TILE_M: tl.constexpr, TILE_N: tl.constexpr, TILE_K: tl.constexpr,  #
    ...
    num_pid_n = tl.num_programs(1) # same as width
    pid_n, pid_m = tl.swizzle2d(pid_n, pid_m, num_pid_n, num_pid_m, GROUP_SIZE_M)
    pidz = tl.program_id(2)
    header = lut + pid_n * 4
    # load header for one row: [offsets, segments, col_id, head_id]
    ...
    # tl.device_print("block_id: ", block_id)
    offs_am = tl.arange(0, TILE_M)
    offs_ak = tl.arange(0, TILE_K)
    pa = A + block_id * stride_ha \
...
def qoft_dsd_matmul(a, b, trans_a, trans_b, trans_c, spdims, block, lut, width, out=None):
    # breakpoint()
    """
    Example: 4D tensor
        spdims = [2, 4, 4]
        ...
        AS0 = 1, AS1 = 2, AS2 = 64, AS3 = 64
        BS0 = 1, BS1 = 2, BS2 = 64, BS3 = 384
        CS0 = 1, CS1 = 2, CS2 = 64, CS3 = 384
    ...
    Example: 3D tensor
        ...
        AS0 = 2, AS1 = 64, AS2 = 64
        BS0 = 2, BS1 = 64, BS2 = 384
        CS0 = 2, CS1 = 64, CS2 = 384
    """
    # breakpoint()
    if a.stride(-2) != 1 and a.stride(-1) != 1:
        a = a.contiguous()
    if b.stride(-2) != 1 and b.stride(-1) != 1:
        b = b.contiguous()
    # shapes / dtypes
    # AS2 = block * spdims[2 if trans_a else 1]
    AS1 = block * spdims[2 if trans_a else 1]
    BS0 = b.size(0)
    # BS1 = b.size(1)
    # BS3 = b.size(2 if trans_b else 3)
    BS2 = b.size(-2 if trans_b else -1)
    dtype = a.dtype
    # allocate output
    CS0 = BS0
    # CS1 = BS1
    CS1 = BS2 if trans_c else AS1
    CS2 = AS1 if trans_c else BS2
    # CS2 = BS3 if trans_c else AS2
    # CS3 = AS2 if trans_c else BS3
    # breakpoint()
    if out is None:
        # c = torch.empty((CS0, CS1, CS2, CS3), dtype=dtype, device=a.device)
        c = torch.empty((CS0, CS1, CS2), dtype=dtype, device=a.device)
    else:
        # assert out.shape == (CS0, CS1, CS2, CS3)
        assert out.shape == (CS0, CS1, CS2)
        c = out
    # meta-parameter heuristics
    TILE_N = 128
    # compute output
    # grid = lambda meta: [cdiv(BS3, meta['TILE_N']), width, BS0]
    grid = lambda meta: [cdiv(BS2, meta['TILE_N']), width]
    _qoft_dsd_kernel[grid](
        a, b, c,  #
        # a.stride(0), a.stride(1), a.stride(3 if trans_a else 2), a.stride(2 if trans_a else 3),  #
        a.stride(0), a.stride(2 if trans_a else 1), a.stride(1 if trans_a else 2),
        # b.stride(0), b.stride(1), b.stride(3 if trans_b else 2), b.stride(2 if trans_b else 3),  #
        ...
        c.stride(0), c.stride(2 if trans_c else 1), c.stride(1 if trans_c else 2),
        # BS3, AS2, lut,  #
        BS2, AS1, lut,  #
...
def qoft_dsd_lut(layout, block, step, trans, device):
    """
    Generates the look-up table for incrementing pointers in the qoft_dsd/qoft_dds matmul.
    ...
    segments = sizes * step
    # pointer increments
    # nnz: for each non-zero block the indices, e.g. nnz has shape [256, 3], [0, 0, 0], [0, 0, 1], ... [1, 127, 127]
    if trans:
...
# -----------------------------
# Dense = Dense x Sparse (qoft_dds)
# -----------------------------
# AB = (B^T A^T)^T


def qoft_dds_matmul(a, b, trans_a, trans_b, trans_c, spdims, block, lut, width, out=None):
    return qoft_dsd_matmul(b, a, not trans_b, not trans_a, not trans_c, spdims, block, lut, width, out=out)
...
class _matmul(torch.autograd.Function):
    ...
    fn = {'qoft_sdd': qoft_sdd_matmul, 'qoft_dsd': qoft_dsd_matmul, 'qoft_dds': qoft_dds_matmul}
...
class matmul:
    ...
    def __init__(self, layout, block, mode, device, trans_a=False, trans_b=False, trans_c=False):
        if mode not in ['qoft_sdd', 'qoft_dsd', 'qoft_dds']:
            raise NotImplementedError('Supported modes are: qoft_sdd, qoft_dsd, qoft_dds')
        self.block = block
        self.mode = mode
        ...
        step = min(block, 32)
        if self.mode == 'qoft_sdd':
            self.c_lut, self.c_width = qoft_sdd_lut(layout, block, device)
            self.da_lut, self.da_width = qoft_dsd_lut(layout, block, step, True, device)
            self.db_lut, self.db_width = qoft_dsd_lut(layout, block, step, False, device)
        if self.mode == 'qoft_dsd':
            self.c_lut, self.c_width = qoft_dsd_lut(layout, block, step, not self.trans_a, device)
            self.da_lut, self.da_width = qoft_sdd_lut(layout, block, device)
            self.db_lut, self.db_width = qoft_dsd_lut(layout, block, step, self.trans_a, device)
        if self.mode == 'qoft_dds':
            self.c_lut, self.c_width = qoft_dsd_lut(layout, block, step, self.trans_b, device)
            self.da_lut, self.da_width = qoft_dsd_lut(layout, block, step, not self.trans_b, device)
            self.db_lut, self.db_width = qoft_sdd_lut(layout, block, device)
